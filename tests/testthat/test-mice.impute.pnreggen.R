...
test_that("pnreggen Imputation is correct length", {
...
test_that("pnreggen isn't run when the data are complete", {
    ...
    y <- bpnreg_draw(x, B)
    ry <- !is.na(y)
    expect_error(mice.impute.pnreggen(y, ry, x))

})

test_that("pnreggen works with data frame input", {
...
test_that("pnreggen works for single missing obervation", {
    N <- 30
    x <- matrix(rnorm(2 * N), ncol = 2)
    B <- matrix(c(3.5, 1, -3, 0, 3, -0.5), ncol = 2, byrow = TRUE)
    y <- bpnreg_draw(x, B)
